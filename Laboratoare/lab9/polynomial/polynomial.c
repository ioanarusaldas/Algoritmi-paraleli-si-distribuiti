	...
	if (rank == MASTER) { // This code is run by a single process
		...
		*/
		...
		for (int i = 0; i < polynomialSize; i++) {
			...
			*/
			//for (int i = nProcesses - 2; i >= 0 ; i -= 1) {
			if( i != MASTER)
		...
		MPI_Send(&a[0], 1,MPI_FLOAT,1 , 1, MPI_COMM_WORLD);
		//printf("Polynom value is %f\n", a[0]);
		MPI_Send(&x, 1, MPI_FLOAT,1 , 1, MPI_COMM_WORLD);

	} else {
		float val, sum,sum_val;
		int x;
		float recv_coef;
		...
		*/
		MPI_Status status;
        MPI_Recv(&recv_coef, 1, MPI_FLOAT, 0, 1, MPI_COMM_WORLD, &status);

        MPI_Recv(&sum, 1, MPI_FLOAT, rank - 1, 1, MPI_COMM_WORLD, &status);
        MPI_Recv(&x, 1, MPI_FLOAT, rank - 1, 1, MPI_COMM_WORLD, &status);

        sum += recv_coef * (pow(x,rank));
		...
		if (rank == nProcesses - 1) {
			...
			MPI_Send(&x, 1, MPI_FLOAT,rank + 1 ,  1, MPI_COMM_WORLD);
